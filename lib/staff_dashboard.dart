...
class _StaffDashboardState extends State<StaffDashboard> {
  ...
  Future<void> _requestCARole() async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Request CA Role'),
        content: const Text(
          'Are you sure you want to request the Chief Accountant role? '
          'Your request will be sent to the Director of Finance for approval.',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      await firestoreService.submitCARequest(
        districtId: _districtData!['districtId'],
        userId: authService.currentUser!.uid,
      );

      setState(() => _hasRequestedCA = true);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('CA role request submitted successfully!'),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Request CA Role Button (if not DOF/CA)
              if (_userData?['role'] != 'DOF' &&
                  _userData?['role'] != 'CA') ...[
                Card(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        Text(
                          'Interested in CA Role?',
                          style: Theme.of(context).textTheme.titleMedium
                              ?.copyWith(fontWeight: FontWeight.bold),
                        ),
                        const SizedBox(height: 8),
                        Text(
                          'Request the Chief Accountant role from your DOF',
                          textAlign: TextAlign.center,
                          style: TextStyle(color: Colors.grey[700]),
                        ),
                        const SizedBox(height: 16),
                        ElevatedButton.icon(
                          onPressed: _hasRequestedCA ? null : _requestCARole,
